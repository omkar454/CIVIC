...
router.put("/:id/status", auth(["officer", "admin"]), async (req, res) => {
  try {
    const { status, comment, media, officerValidationPass, similarityScore, officerValidationStatus, isInauthentic, isStrictDuplicate } = req.body;
    const user = req.user;
    ...
    if (user.role === "officer") {
      ...
      if (status === "In Progress") {
        ...
        console.log(`🔍 [Backend] Resolve Attempt: SimilarityScore=${similarityScore}, ValidationStatus=${officerValidationStatus}, IdentityMatch=${isStrictDuplicate}`);
        
        const isFraud = similarityScore > 0.98 || isStrictDuplicate;
        ...
        const isAuditFail = officerValidationPass === false;

        // 🚨 CASE 0: Inauthentic Content (Cat/Dog/Meme check)
        if (isInauthentic) {
           const result = await User.autoWarn(
            user._id,
            `Inauthentic Content Alert: Officer tried to upload a non-civic photo (detected as unrelated content).`,
            "FakeImage",
            null, 
            report._id
          );
          ...
          report.statusHistory.push({
            ...
            note: "⚠️ AI SECURITY REJECTION: Inauthentic/Non-Civic photo detected. Action blocked.",
            actorRole: "admin",
            at: new Date(),
            autoGenerated: true
          });
          ...
          return res.status(403).json({
            ...
            error: "AI Security Alert: Inauthentic content detected.",
            abuseData: result
          });
        }
        ...
        if (isFraud) {
          ...
          const result = await User.autoWarn(
            user._id,
            `Fraud Attempt: Officer tried to upload a duplicate photo.`,
            "FRAUD_ATTEMPT",
            null, 
            report._id
          );
          ...
          report.statusHistory.push({
            status: report.status,
            note: "⚠️ AI FRAUD REJECTION: CLIP identified a duplicate photo. Action blocked.",
            actorRole: "admin",
            at: new Date(),
            autoGenerated: true
          });
          ...
          return res.status(403).json({
            message: result.message,
            error: "Fraudulent photo detected.",
            abuseData: result
          });
        } else if (isMismatch) {
          // 📍 AI MISMATCH REJECTION: Officer is at wrong location.
          const result = await User.autoWarn(
            user._id,
            `Location Mismatch: Officer at incorrect location (${(similarityScore * 100).toFixed(1)}%).`,
            "LOCATION_MISMATCH",
            null,
            report._id
          );
          ...
          report.statusHistory.push({
            status: report.status,
            note: `⚠️ AI MISMATCH REJECTION: Location similarity is too low (${(similarityScore * 100).toFixed(1)}%).`,
            actorRole: "admin",
            at: new Date(),
            autoGenerated: true
          });
          ...
          return res.status(403).json({
           ...
           // 👁️ AUDIT REJECTION: Issue still detected in after photo.
           const result = await User.autoWarn(
            user._id,
            `Incomplete Resolution: ${report.category} still detected in resolution photo.`,
            "INCOMPLETE_WORK",
            null,
            report._id
          );
          ...
          report.statusHistory.push({
            status: report.status,
            note: `⚠️ AI AUDIT REJECTION: ${report.category} still detected at location. Action blocked.`,
            actorRole: "admin",
            at: new Date(),
            autoGenerated: true
          });
          ...
          return res.status(403).json({
            message: result.message,
            error: "Issue removal not verified by AI scan.",
            abuseData: result
          });
